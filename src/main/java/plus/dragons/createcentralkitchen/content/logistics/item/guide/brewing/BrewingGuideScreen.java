...
public class BrewingGuideScreen extends AbstractSimiContainerScreen<BrewingGuideMenu> {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float v, int i, int i1) {
        ...
        int guideX = getLeftOfCentered(WINDOW_WIDTH);
        int guideY = topPos;
        renderGuide(guiGraphics, guideX, guideY);
        //Title
        guiGraphics.drawCenteredString(font, title, guideX + WINDOW_WIDTH / 2, guideY + 5, 0xFFFFFF);
        //Guide Icon
    ...
    private void renderGuide(@NotNull GuiGraphics guiGraphics, int x, int y) {
        ...
        if (level == null || status <= 0) {
            guiGraphics.blit(TEXTURE, x + 56, y + 24, 0, 80, 24, 24);
        } else {
            int time = ((int) level.getGameTime() / 5 % 3) + 1;
            guiGraphics.blit(TEXTURE, x + 56, y + 24, 24 * time, 80, 24, 24);
        }
        //if (needWater)
        //    guiGraphics.blit(TEXTURE, x + 11, y + 32, 176, 80, 5, 32);
        guiGraphics.blit(TEXTURE, x + 48, y + 66, 96, 80 + status * 6, 80, 6);
    }
    ...
    protected void renderTooltip(@NotNull GuiGraphics guiGraphics, int x, int y) {
        if (this.menu.getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
            if (this.hoveredSlot.index == 2) {
                ...
                guiGraphics.renderTooltip(font, this.hoveredSlot.getItem(), x, y);
            }
        }
    }
