...
public class BlazeStoveScenes {
    ...
    public static void automation(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        if (scene instanceof CreateSceneBuilder createScene) {
            createScene.world().setKineticSpeed(util.select().everywhere(), 0);
        }
        scene.showBasePlate();
        scene.idle(5);
        ...
        scene.overlay().showOutlineWithText(stoveSel, 70)
                .attachKeyFrame()
        ...
        scene.overlay().showControls(util.vector().centerOf(3, 1, 1), Pointing.DOWN, 30)
                .rightClick();
        scene.idle(40);

        scene.overlay().showControls(util.vector().centerOf(3, 1, 1), Pointing.LEFT, 30)
                .withItem(ModItems.GLOW_BERRY_CUSTARD.get().getDefaultInstance());
        scene.idle(40);
        ...
        scene.overlay().showControls(util.vector().centerOf(3, 1, 1), Pointing.DOWN, 30)
                .withItem(FDItemEntries.COOKING_GUIDE.asStack())
                .rightClick();
        scene.idle(40);
        ...
        scene.idle(10);
        if (scene instanceof CreateSceneBuilder createScene) {
            createScene.world().setKineticSpeed(util.select().fromTo(0, 0, 0, 2, 1, 2), 64);
        }
        scene.idle(10);
        ...
        if (scene instanceof CreateSceneBuilder createScene) {
            for (var item : inputItems) {
                createScene.world().instructArm(rightArmPos, ArmBlockEntity.Phase.MOVE_TO_INPUT, ItemStack.EMPTY, 0);
                createScene.idle(12);
                createScene.world().instructArm(rightArmPos, ArmBlockEntity.Phase.SEARCH_OUTPUTS, item, -1);
                createScene.idle(8);
                createScene.world().instructArm(rightArmPos, ArmBlockEntity.Phase.MOVE_TO_OUTPUT, item, 0);
                createScene.idle(12);
                createScene.world().instructArm(rightArmPos, ArmBlockEntity.Phase.SEARCH_INPUTS, ItemStack.EMPTY, -1);
                createScene.idle(8);
            }
        }
        ...
        scene.idle(10);
        if (scene instanceof CreateSceneBuilder createScene) {
            createScene.world().setKineticSpeed(util.select().fromTo(4, 0, 0, 6, 1, 2), 64);
        }
        ...
        if (scene instanceof CreateSceneBuilder createScene) {
            createScene.world().instructArm(leftArmPos, ArmBlockEntity.Phase.MOVE_TO_INPUT, ItemStack.EMPTY, 0);
            createScene.idle(12);
            createScene.world().instructArm(leftArmPos, ArmBlockEntity.Phase.SEARCH_OUTPUTS, result, -1);
            createScene.idle(8);
            createScene.world().instructArm(leftArmPos, ArmBlockEntity.Phase.MOVE_TO_OUTPUT, result, 0);
            createScene.idle(12);
            createScene.world().instructArm(leftArmPos, ArmBlockEntity.Phase.SEARCH_INPUTS, ItemStack.EMPTY, -1);
            createScene.idle(18);
        }
    }
