...
public class BasketScenes {
    ...
    public static void intro(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        scene.overlay().showOutlineWithText(util.select().position(chestBasketPos), 40)
            .attachKeyFrame()
    ...
    public static void belt_interaction(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        if (scene instanceof CreateSceneBuilder createScene) {
            createScene.world().modifyKineticSpeed(util.select().everywhere(), f -> f / 2f);
        }
        scene.idle(5);
        ...
        if (scene instanceof CreateSceneBuilder createScene) {
            for (int i = 0; i < 8; i++) {
                createScene.idle(8);
                createScene.world().removeItemsFromBelt(exitBeltPos);
                createScene.world().flapFunnel(exitBeltPos.above(), false);
                if (i == 2)
                    createScene.rotateCameraY(70);
                if (i < 6)
                    createScene.world().createItemOnBelt(entryBeltPos, Direction.EAST, vegetables[i]);
            }
        }
