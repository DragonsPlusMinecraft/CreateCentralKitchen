...
public class AutomaticModLoadSubscriber {
    ...
    public static void load(FMLModContainer mod, Class<?> modClass) {
        ...
        for (var target : targets) {
            ...
            final List<ModAnnotation.EnumHolder> distHolder = (List<ModAnnotation.EnumHolder>) target.annotationData().getOrDefault("value", DEFAULT_DIST);
            ...
            if (!Mods.isLoaded(requiredModId))
                continue;
            final ModAnnotation.EnumHolder busHolder = (ModAnnotation.EnumHolder) target.annotationData().getOrDefault("bus", DEFAULT_BUS);
