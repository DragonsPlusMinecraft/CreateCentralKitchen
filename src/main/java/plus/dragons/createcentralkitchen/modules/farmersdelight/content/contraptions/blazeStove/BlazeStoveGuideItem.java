...
public abstract class BlazeStoveGuideItem<G extends BlazeStoveGuide> extends Item implements MenuProvider {
    ...
    public void appendGuideTooltip(ItemStack stack, List<Component> tooltip, boolean goggle) {
        ...
        if (result.isEmpty()) {
            var text = CentralKitchen.LANG.translate("gui.goggles.blaze_stove.no_result").style(ChatFormatting.RED);
            if (goggle)
                text.forGoggles(tooltip);
            else
                text.addTo(tooltip);
        } else {
            var text = CentralKitchen.LANG.translate("gui.goggles.blaze_stove.recipe_result");
            var itemName = CentralKitchen.LANG.itemName(result).style(ChatFormatting.GRAY);
            if (goggle) {
                text.forGoggles(tooltip);
